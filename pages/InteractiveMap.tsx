    ...
    <div className="bg-gray-50 min-h-screen pb-32 font-sans overflow-x-hidden">
      ...
      <header className="bg-white px-6 pt-12 pb-6 sticky top-0 z-40 shadow-sm border-b border-gray-100">
        <div className="flex items-center justify-between mb-6">
          ...
          <button 
            ...
            className="px-4 h-10 bg-emerald-50 rounded-2xl flex items-center gap-2 text-emerald-600 border border-emerald-100 font-bold text-xs uppercase tracking-wide hover:bg-emerald-100 transition-colors"
          >
            {viewMode === 'list' ? (<><MapIcon size={16} /> Mapa</>) : (<><Grid size={16} /> Lista</>)}
          </button>
        </div>
      ...
      <div className="p-6">
        {viewMode === 'list' ? (
          ...
          <div className="bg-white rounded-[40px] w-full h-[65vh] border-4 border-emerald-100 shadow-2xl relative overflow-hidden">
            <DirectoryMapbox activeCategory={activeCategory} searchQuery={searchQuery} />
          </div>
        )}
      </div>
